d3.slider = function module() {
  ...
  function slider(selection) {
    selection.each(function() {
      ...
      if(typeof customDragStart == 'function') {
        dragBehaviour.on("dragstart", customDragStart);
      }

      dragBehaviour.on("drag", slider.drag);

      if(typeof customDragStop == 'function') {
        dragBehaviour.on("dragend", customDragStop);
      }
  ...
  slider.move = function(pos, animated) {
    var l,u;
    ...
    // Move dragger
    var customDragger = svg.selectAll(".dragger").data(values);
    ...
    if(animated) {
      ...
      customDragger = customDragger.transition()
                                   .duration(duration)
                                   .ease('linear');
    }

    customDragger.attr("transform", function(d) {
      return "translate(" + scale(d) + ")";
    });
